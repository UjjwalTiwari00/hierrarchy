...
/** Single entry in the Materialized Path cache — built once in index.ts */
export interface MPCacheEntry {
  path: string;          // "GBS1|||GBS2|||GBS3"
  label: string;         // "GBS3"  (last segment only)
  depth: number;         // 0-indexed — Level1 = 0
  parentPath: string;    // "GBS1|||GBS2"  (empty string for roots)
  strategyName?: string; // only on leaf entries
  strategyId?: string;   // only on leaf entries
  strategyTypeKey: number;
  level1Value: string;   // for filterName matching
  recordId: string;      // for filterId matching
}
...
interface Props {
  ...
  mpCache?: Map<string, MPCacheEntry>;  // ← NEW: pre-built MP cache from index.ts
  onSelectionChange?: (names: string[], ids: string[]) => void;
  onRefresh?: () => void;
}

// ─────────────────────────────────────────────────────────────
// buildTreeFromCache — fast path when MP cache is available
// Avoids raw records scan entirely. O(unique paths) instead of O(35k rows).
// ─────────────────────────────────────────────────────────────
function buildTreeFromCache(
  cache: Map<string, MPCacheEntry>,
  filterId: string,
  filterName: string,
  strategyTypeKey: number
): TreeNode[] {
  ...
  // Pass 1: create TreeNode for every cache entry that passes filters
  for (const [path, entry] of cache) {
    if (filterId     && entry.recordId !== filterId)                                  continue;
    if (filterName   && entry.level1Value.toLowerCase() !== filterName.toLowerCase()) continue;
    if (strategyTypeKey > 0 && entry.strategyTypeKey !== strategyTypeKey)             continue;
  ...
  // Pass 2: wire parent → child using parentPath stored in each entry
  const roots: TreeNode[] = [];
  nodeMap.forEach((node) => {
    const entry = cache.get(node.path);
    if (!entry) return;
    ...
    if (!entry.parentPath) {
      ...
      if (parent) {
        ...
        // Parent was filtered out — treat this node as a root
        roots.push(node);
      }
    }
  });
...
// ─────────────────────────────────────────────────────────────
// buildTree — fallback path (no cache yet, e.g. during paging)
// ─────────────────────────────────────────────────────────────
function buildTree(
  records: RecordSnapshot[],
  maxLevels: number,
...
export const HierarchyTree: React.FC<Props> = ({
  ...
  // ─────────────────────────────────────────────────────────────
  // Build tree:
  //   • If MP cache is available (final render) → fast path
  //   • Otherwise fall back to raw records scan (during paging)
  // ─────────────────────────────────────────────────────────────
  const tree = useMemo(() => {
    if (mpCache && mpCache.size > 0) {
      // Fast path: cache is ready, no raw records scan needed
      return buildTreeFromCache(mpCache, filterId, filterName, strategyTypeKey);
    }
    // Fallback: cache not ready yet (still paging), scan raw records
    return buildTree(records, maxLevels, filterId, filterName, strategyTypeKey);
  }, [mpCache, records.length, maxLevels, filterId, filterName, strategyTypeKey]);
